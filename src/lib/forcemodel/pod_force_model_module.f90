...
contains
    ...
    subroutine compute_acceleration(position, velocity, time, acceleration)
        ...
        real(DP), dimension(3) :: acc_grav, acc_drag, acc_srp, acc_rel
        ...
        if (config%use_relativity) then
            call compute_post_newtonian(position, velocity, time, acc_rel)
        else
            acc_rel = 0.0_DP
        end if
    ...
    subroutine compute_solar_radiation_pressure(position, time, acceleration, Cr, SMR, RP)
     ...
     ! ======================================================================
    ! 后牛顿相对论效应 (一级)，只考虑日、地、月
    ! 与所提供代码中的 f_PNE 实现相同
    ! ======================================================================
    subroutine compute_post_newtonian(position, velocity, time, acc_rel)
        real(DP), dimension(3), intent(in) :: position, velocity
        real(DP), intent(in) :: time
        real(DP), dimension(3), intent(out) :: acc_rel
        
        integer :: i
        integer, parameter :: rel_bodies(3) = [3, 10, 11]   ! 地球、月球、太阳
        real(DP), dimension(3) :: body_pos, body_vel, r_rel, v_rel
        real(DP) :: r, v2, r_dot_v
        ...
        do i = 1, 3
            ...
            r = norm2(r_rel)
            if (r < 1.0e-12_DP) cycle   ! 避免奇异
            
            v2 = dot_product(v_rel, v_rel)
            r_dot_v = dot_product(r_rel, v_rel) / r   ! 径向速度 (v_rel·r̂)
    ...
    end subroutine compute_post_newtonian
