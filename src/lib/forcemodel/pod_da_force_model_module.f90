...
contains
    ...
    subroutine da_compute_central_gravity(position, acceleration)
        type(AlgebraicVector), intent(in) :: position
        type(AlgebraicVector), intent(out) :: acceleration
        
        real(DP) :: r_mag, r_mag3
        
        r_mag = sqrt(sum(position%elements**2))
        r_mag3 = r_mag**3
        
        ! 中心引力: a = -μ * r / |r|³
        acceleration = -config%gravitational_constant * position / r_mag3
    end subroutine da_compute_central_gravity
    
    subroutine da_compute_j2_perturbation(position, acceleration)
        type(AlgebraicVector), intent(in) :: position
        type(AlgebraicVector), intent(out) :: acceleration
    ...
    end subroutine da_compute_j2_perturbation
    ...
    subroutine da_compute_atmospheric_density(altitude, density)
    ...
    end subroutine da_compute_atmospheric_density
    
    subroutine da_compute_solar_radiation_pressure(position, acceleration)
    ...
    subroutine da_compute_third_body_perturbation(position, time, acceleration)
        real(DP), dimension(3), intent(in) :: position
        real(DP), intent(in) :: time
        real(DP), dimension(3), intent(out) :: acceleration
    ...
    end subroutine da_compute_third_body_perturbation
    
    subroutine da_compute_third_body_gravity(position, body_position, body_mu, acceleration)
    ...
    end subroutine da_compute_third_body_gravity
    
    subroutine da_set_force_model_options(use_j2, use_drag, use_srp, use_third_body)
    ...
    end subroutine da_set_force_model_options
    
    subroutine da_print_force_model_status()
        write(*, *) '=== 力模型状态 ==='
        write(*, *) '中心引力: 启用'
    ...
    end subroutine da_print_force_model_status
