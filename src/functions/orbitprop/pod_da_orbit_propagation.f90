...
contains
    ...
    subroutine get_initial_orbit_state(initial_state, epoch, frame)
        type(orbit_state), intent(out) :: initial_state
        real(DP), intent(out) :: epoch
        character(len=*), intent(out) :: frame
        character(len=*) :: utc_string
        ...
        write(*, *) '请输入历元时间 (UTC):'
        utc_string = get_user_string('UTC (e.g., 2024-03-09T12:00:00): ')
        call str2et(trim(utc_string), epoch)
    ...
    subroutine propagate_orbit(initial_state, final_time, step_size, integrator_choice, result)
        ...
        select case (integrator_choice)
            case (1)
                call adaptive_step_integrate(initial_state%state, initial_state%epoch, final_time, config%rkf45_max_step, & 
                        config%rkf45_tolerance, integrator_choice, result%times, result%states, n_steps_45)
            case (2)
                call adaptive_step_integrate(initial_state%state, initial_state%epoch, final_time, config%rkf78_max_step, & 
                                config%rkf78_tolerance, integrator_choice, result%times, result%states, n_steps_78)
