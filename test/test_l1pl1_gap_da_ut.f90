!> @file test_l1pl1_gap_da_ut.f90
!> @brief L1PL-1 init2 OPM 传播至 2027-01-12T15:00:00: DA-MC(4) + UKF(alpha=1.0), GMM n=3/5
!> UKF 使用 pod_filter_ut_module 的 time_update，alpha=1.0
!> 输出至 output/0627_L1PL1_DAMC4_UKF_GMM/
program test_l1pl1_gap_da_ut
    ...
    opm_file    = 'OPM/L1PL-1/L1PL-1_init2.opm.json'
    output_dir  = 'output/0627_L1PL1_DAMC4_UKF_GMM'

    write(*,*) '=================================================='
    write(*,*) '  L1PL-1 init2: DA-MC(4) vs UKF(a=1.0)'
    write(*,*) '  目标: 2027-01-12T15:00:00 UTC'
    ...
    call write_scatter_txt(trim(output_dir) // '/L1PL-1_DA_MC4_particles.txt', &
                           final_da%samples, size(final_da%samples, 2))
    call write_mean_cov_txt(trim(output_dir) // '/L1PL-1_DA_MC4_mean_cov.txt', &
    ...
    call write_json_opm(trim(output_dir) // '/L1PL-1_DA_MC4', &
                        final_da%mean, final_da%cov, &
                        rms=0.0_DP, obj_id='L1PL-1', et_last=target_et)
    ...
    call write_json_opm(trim(output_dir) // '/L1PL-1_DA_MC4_GMM3', &
    ...
    call write_json_opm(trim(output_dir) // '/L1PL-1_DA_MC4_GMM5', &
                        final_da%mean, final_da%cov, gmm_state=gmm5, &
                        rms=0.0_DP, obj_id='L1PL-1', et_last=target_et)

    ! ===== UKF time_update (alpha=1.0) =====
    write(*,*) ''
    write(*,*) '--- UKF time_update (alpha=1.0) ---'
    ...
    call write_mean_cov_txt(trim(output_dir) // '/L1PL-1_UKF_mean_cov.txt', &
                            ukf_mean, ukf_cov)

    call write_json_opm(trim(output_dir) // '/L1PL-1_UKF', &
                        ukf_mean, ukf_cov, &
                        rms=0.0_DP, obj_id='L1PL-1', et_last=target_et)
...
end program test_l1pl1_gap_da_ut
